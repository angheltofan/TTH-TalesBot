  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center relative landscape:w-1/2 landscape:h-full py-4">
      <div className="text-center mb-6 landscape:hidden animate-fade-in-down">
        <h1 className="text-3xl font-normal tracking-tight drop-shadow-sm text-white">
          TTH - Tale Bot
        </h1>
        <p className="text-sm font-normal text-cyan-50 mt-1 opacity-90">
          Asistentul de joacă al Tales&Tech
        </p>
      </div>

      <div className="relative flex items-center justify-center w-full max-w-[280px] aspect-square">
        <div
          className={`absolute inset-0 bg-cyan-200/30 rounded-full blur-[60px] transition-all duration-700 ${
            isConnected ? "scale-110 opacity-100" : "scale-90 opacity-40"
          }`}
        ></div>
        ...
        <div
          className={`
                relative z-10 w-full h-full rounded-full bg-white border-4 border-cyan-200 shadow-2xl overflow-hidden flex items-center justify-center
                transition-transform duration-300 ease-out
                ${isConnected ? "animate-float" : ""}
            `}
          style={{
            transform: isConnected
              ? `scale(${1 + Math.min(audioVolume * 0.5, 0.2)})`
              : "scale(1)",
          }}
        >
          <img
